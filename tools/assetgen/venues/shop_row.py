...
def _shop_front(ctx, asset_id, width, x0, key, rng):
    ...
    ctx.emit(door)
    ctx.entity(f"{asset_id}.door.01", f"door.{key}", (door_x, 0.35, zf - 0.15),
               cell="D5", verbs=["enter"])
    ...
    ctx.entity(f"{asset_id}.counter.01", f"vendor.{key}",
...
def build(ctx: VenueContext, asset_id="hm.shop"):
    ...
    for (key, w, up_h, style, pitch, sign_mat) in SHOPS:
        ...
        ctx.emit(K.stone_plinth(w + 0.1, DEPTH + 0.2, 0.35), "stone")
